...
When I was in middle school (_in the United States_), I tried to install Pterodactyl and ended up
...
It's a shame though that the [official documentation](https://pterodactyl.io/panel/1.0/getting_started.html)
...
These are some basic requirements and if you know what you are doing, this guide is still fine to follow just adjust accordingly:
...
_Why open 8080/tcp and 8080/udp when we could just use 443? Pterodactyl is rather picky and using 443 exclusively
does not play well with Caddy. It is doable though and I encourage you to give it a try if you think you've got the chops._

> **NOTE**: If the `docker compose` command isn't resolving for you, you might need to use `docker-compmose` instead.
...
You won't be able to reach the panel yet because we haven't set up Caddy yet. We'll come back to that.
...
Don't start `wings` quite yet, we'll get back to that soon enough. (It needs to be configured first before starting in order to do anything useful!)
...
in is the `Caddyfile` (see file structure):
...
Change the domains to the domains to your domains but
...
These commands on Linux will increase the maximum UDP buffer size, which you might want to do for [performance reasons](https://github.com/quic-go/quic-go/wiki/UDP-Buffer-Sizes) for QUIC (which runs on UDP) and is enabled by default in Caddy:
...
This guide is especially finnicky and difficult, ask questions in the comments or [contact me](https://github.com/imkunet) directly.
